...
void mySort(T arr[], int len)
{
	for (int i = 0; i < len; i++)
	{
		...
		{
			//认定的最大值比遍历的数值要小，则j下标的元素才是真正的最大值
            if (arr[max] < arr[j])
			{
				max = j;
			}
...
void test01()
{
    ...
    cout << myAdd02<char>(a, c) << endl; //正确，如果用显示指定类型，可以发生隐式类型转换
}
...
void test01()
{
	...
	char c1 = 'a';
	char c2 = 'b';
	myPrint(c1, c2); //调用函数模板，而不是普通函数（普通函数需要隐式类型转换）
}
...
//类模板
//类模板在模板参数列表中可以有默认参数 (设置AgeType默认是int)
...
void test01()
{
	...
	m.fun1();    //调用Person1类的showPerson1()

	//m.fun2();  
    /*
    调用Person1类的showPerson2()，但是Person1类并没有showPerson2()，
...
class Son :public Base<int> //必须指定一个类型,这样父类中的T的类型就只能是int了
{
